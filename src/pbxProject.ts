...
class pbxProject extends EventEmitter {
  filepath: string;
  ...
  removeFromPbxFileReferenceSection(file) {
    var i;
  ...
  pbxItemByComment(name, pbxSectionName) {
  ...
  addBuildProperty(prop, value, build_name) {
  ...
  removeBuildProperty(prop, build_name) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection()),
      key,
      configuration;
   ...
   */
  updateBuildProperty(prop, value, build) {
  ...
  getTarget(productType) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (
        nativeTargets[targetUuid]["productType"] ===
        '"' + productType + '"'
      ) {
        // Get pbxNativeTarget
  ...
  pbxCreateGroupWithType(name, pathName, groupType) {
    //Create object
    var model = {
      isa: '"' + groupType + '"',
      children: [],
